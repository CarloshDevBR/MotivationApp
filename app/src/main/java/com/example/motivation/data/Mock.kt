...
class Mock {
    ...
    private val mListPhrase = listOf(
        ...
        Phrase(
            "Você não é derrotado quando perde, você é derrotado quando desiste!", happy, langPt
        ),
        ...
        Phrase(
            "Riscos devem ser corridos, porque o maior perigo é não arriscar nada!", sunny, langPt
        ),
        Phrase("Not knowing it was impossible, he went there and did it.", happy, langEn),
        Phrase(
            "You are not defeated when you lose, you are defeated when you give up!", happy, langEn
        ),
        ...
        Phrase(
            "Tu n'es pas vaincu quand tu perds, tu es vaincu quand tu abandonnes!", happy, langFr
        ),
    ...
    fun getPhrash(id: Int, lang: String): String {
        if (id == all) return mListPhrase.filter { it.lang == lang }.shuffled().first().description

        return mListPhrase.filter { it.categoryId == id }.shuffled().first().description
    }
}